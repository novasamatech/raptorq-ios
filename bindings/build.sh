...
# Copy actual header
cp "./generated/raptorq/raptorq.h" "$headers_nested/"

# Create module.modulemap that references the subfolder
cat <<EOF > "$headers_nested/module.modulemap"
module $lib_name {
    header "$lib_name/raptorq.h"
    export *
}
EOF

# Symlink raptorq.h at top-level (so xcodebuild accepts it)
ln -s "$lib_name/raptorq.h" "$headers_root/raptorq.h"
